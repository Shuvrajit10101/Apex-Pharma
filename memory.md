...
- **Phase:** **Phase 1 (Core MVP) — COMPLETE & merged. Phase 2 (Round out) — IN PROGRESS** (2(a)+2(b)+2(c) merged; starting 2(d) GSTR-1 export). Phase 1: 1(a) auth · 1(b) masters · nav-shell · 1(c) purchase/GRN · 1(d) billing · 1(e) invoice+settings · 1(f) reports · 1(g) backup/restore — plus .NET 10 (LTS) upgrade + NU1903 fix.
...
- **Repo:** live on GitHub, **PUBLIC**; `main` @ `ea4e5ba` (2(a) #16 · 2(b) #17 · 2(c) #18 all merged, clean, synced with `origin/main`); **branch protection ENABLED** (PR required + `Build & test (Release)` check; no force-push/deletion; admin can override). CI green (.NET 10). **341 tests, no vulnerable packages.** ⚠️ **Canonical working copy = `C:\dev\Apex-Pharma`** (open the project HERE — the OneDrive `Desktop\pharma` copy is a stale leftover, do NOT use).
- **Now:** ▶️ **Session 3 (2026-07-02) — ULTRACODE on.** **Phase 2(c) MERGED** → `main` @ **`ea4e5ba`** (PR #18, CI green, branch deleted; 341 tests). 2(a) #16 · 2(b) #17 · 2(c) #18 done. **Starting 2(d) — GSTR-1 / GST-return export** (build on the existing GST/HSN summary in `ReportService`). Design → implement → review → PR → merge.
...
- **Next (Phase 2, ordered) — AT (d):** ✅(a) returns · ✅(b) adjustments/expiry · ✅(c) ledgers/statements · **(d) GSTR-1 export ← IN PROGRESS** · (e) day-end + Cashier view · (f) Schedule-X strict register/dual-Rx · (g) pre-go-live polish (IST report dates, barcode) → then owner review of Phase 2.
...
**Phase 2 (Round out) — in progress. Done: ✅(a) returns (PR #16) · ✅(b) adjustments/expiry write-off (PR #17) · ✅(c) supplier & customer ledgers/statements (PR #18). Remaining, ordered:**
1. **(d) GSTR-1 / GST-return export ← IN PROGRESS** — build on the existing GST/HSN summary (`ReportService`); a GSTR-1-shaped export for the accountant.
2. **(e) Day-end + Cashier view** — cash reconciliation; Cashier-scoped own-sales / day-end reports.
3. **(f) Schedule-X strict register / dual-Rx** — narcotic register beyond the current H1-style doctor+Rx gate.
4. **(g) Pre-go-live polish** — IST-aware report day boundaries (UTC `BillDate` vs local date shifts evening sales; now also covers `LedgerMath`); barcode polish; manual restore drill on the counter PC.
5. **→ then bring Phase 2 to owner review.**
